...
interface ElementDetailsProps extends Item {
  idx: number;
}

const elementDetails = ref<ElementDetailsProps>();
...
const showDetails = (item: Item, idx: number) => {
  elementDetails.value = { ...item, idx };
  ...
  <div class="inventory">
    <div class="inventory__inner">
      <ul class="inventory__cells">
        <li
          class="inventory__cell drop-zone"
          :data-index="idx"
        ...
        >
          <InventoryElement
            v-if="cell.item"
            :data-index="idx"
            @click="showDetails(cell.item, idx)"
            @click.stop
          >
            <div class="circle" :style="{ backgroundColor: cell.item.color }">
              <span>{{ cell.item.id + 1 }}</span>
            </div>
            <span class="quantity" v-if="cell.item.quantity > 1">{{
              cell.item.quantity
            }}</span>
          </InventoryElement>
        </li>
    ...
    <Button
      ...
      @click="
        clearCells();
        statusNotfAnimate('Ячейки очищены', 'failed');
      "
      >Очистить ячейки</Button
    >
    <div class="status-notifications">
      <transition-group name="fade-status">
        <div
          class="status"
          v-for="(notification, idx) in statusNotificationsArr"
          :key="idx"
        >
